...
class JsonApiClient {
  ...
  /// Use [headers] to pass extra HTTP headers.
  Future<CustomResponse<ToOne>> fetchToOne(Uri uri, {Map<String, String> headers}) =>
  ...
  Future<CustomResponse<ResourceData>> createResource(Uri uri, Resource resource,
          {Map<String, String> headers}) =>
  ...
  Future<CustomResponse> deleteResource(Uri uri, {Map<String, String> headers}) =>
      _call(_delete(uri, headers), null);

  /// Updates the resource via PATCH query.
  ///
  /// https://jsonapi.org/format/#crud-updating
  Future<CustomResponse<ResourceData>> updateResource(Uri uri, Resource resource,
          {Map<String, String> headers}) =>
  ...
  /// with id = null.
  Future<CustomResponse<ToOne>> deleteToOne(Uri uri, {Map<String, String> headers}) =>
  ...
  Future<CustomResponse<ToMany>> replaceToMany(Uri uri, List<Identifier> identifiers,
          {Map<String, String> headers}) =>
  ...
  ///
  /// https://jsonapi.org/format/#crud-updating-to-many-relationships
  Future<CustomResponse<ToMany>> addToMany(Uri uri, List<Identifier> identifiers,
          {Map<String, String> headers}) =>
      _call(_post(uri, headers, _build.toManyDocument(identifiers)),
          ToMany.decodeJson);

  Future<Response> _get(Uri uri, Map<String, String> headers) => httpClient.get(uri.toString(),options: Options(headers: headers));

  Future<Response> _post(Uri uri, Map<String, String> headers, Document doc) => httpClient.post(uri.toString(),data: json.encode(doc));

  Future<Response> _delete(Uri uri, Map<String, String> headers) => httpClient.delete(uri.toString());

  Future<Response> _patch(uri, Map<String, String> headers, Document doc) => httpClient.patch(uri.toString(),data: json.encode(doc));
  ...
  Future<Response> res, D decodePrimaryData(Object _)) async {
    ...
    if (response.data==null) {
      return CustomResponse(response.statusCode, response.request.headers);
    }
    if (StatusCode(response.statusCode).isPending) {
      return CustomResponse(response.statusCode, response.request.headers,
          asyncDocument: response.data == null
              ? null
              : Document.decodeJson(response.data, ResourceData.decodeJson));
    }
    return CustomResponse(response.statusCode, response.request.headers,
        document:
            response.data == null ? null : Document.decodeJson(response.data, decodePrimaryData));
  }
}
